...
pub fn sell_free_token(ctx: Context<SellFreeToken>, sell_amount: u64) -> Result<()> {
    ...
    token::transfer(
        CpiContext::new_with_signer(
            ...
            token::Transfer {
                ...
                to: ctx.accounts.requester_utility_token.to_account_info(),
                authority: ctx.accounts.knowledge_swap.to_account_info(),
            },
            signer,
        ),
        return_utility_token_amount,
    )?;
    ...
    emit!(SellFreeTokenEvent {
        knowledge_token_mint: knowledge_token_mint_key,
        requester_utility_token: ctx.accounts.requester_utility_token.key(),
        knowledge_token_amount: sell_amount,
        utility_token_amount: return_utility_token_amount,
        trade_fee_amount: swap_fee,
    });
...
pub struct SellFreeToken<'info> {
    ...
    pub requester_utility_token: Box<Account<'info, TokenAccount>>,
