...
export const runTest = async (
  ...
  freeTokenSeller: anchor.web3.Keypair,
  needClaim: boolean,
  ...
  program.addEventListener("sellFreeTokenEvent", (event) => {
    console.log("🚀 ~ event:", event);
  });

  if (config.name === "LOCAL") {
    await requestAirdrop(freeTokenSeller.publicKey);
  }
  ...
  const requestorUtilityTokenAccount = await getOrCreateAssociatedTokenAccount(
    provider.connection,
    freeTokenSeller,
    knowledgeSwap.utilityTokenMint,
    freeTokenSeller.publicKey
  );
  ...
  if (needClaim) {
    ...
    const expectedFeeVaultAmount =
      ...
      differentialUtilityAmountForClaimedAmount +
      sellFreeTokenFee;
    ...
    const actualFeeVaultAmount = Number(feeVaultTokenAccountAfter.amount);
    const expectedFeeVaultAmount =
      Number(feeVaultTokenAccountBefore.amount) + sellFreeTokenFee;
